...
- **validate_raw_counts()** — now validates all 57 datasets (tolerance thresholds stored in config, not hardcoded)
...
**Expected:** Pipeline handles all 57 datasets, adds a new one with 1-line registry change.
...
|-----------|-----------|
| Socrata schema drift (57 datasets, different schemas) | Defensive column discovery (already proven); candidate lists per dataset type |
...
- ✅ All 57 datasets load end-to-end in <5 minutes (vs <30s for 4 datasets)
...
- ✅ All validation checks scale to 57 datasets
- ✅ Audit trails for all 57 datasets
