...
NAV = [
    ...
    { "label": "Quantum Optimizer", "icon": "bi bi-lightning-charge", "path": "/quantum", "badge": "AI+" },
...
def build_sidebar(collapsed: bool = False) -> html.Div:
    ...
    return html.Div(
        ...
        children=[
            ...
            html.Div([
                html.Div([
                    html.I(className="bi bi-buildings-fill", style={"fontSize": "1.2rem", "color": "white"}),
                ], className="nyc-logo-icon"),
                html.Div([
                    html.Div("NYC DOT", className="nyc-logo-title"),
                    html.Div("Sidewalk Toolkit", className="nyc-logo-sub"),
                ], className="nyc-logo-text") if not collapsed else None,
            ...
            html.Div([
                html.Button(
                    [
                        html.I(className="bi bi-chevron-left" if not collapsed else "bi bi-chevron-right"),
    ...
    return html.Header(
        ...
        children=[
            html.Div([
                ...
                html.Div("NYC DOT — Sidewalk Inspection & Management", className="nyc-header-title d-none d-sm-block"),
...
app.layout = html.Div(
    ...
    **{"data-theme": "dark"},
    children=[
        ...
        dcc.Store(id="auth-store",      data={"is_authenticated": True, "user": "Richard Yudkiss"}, storage_type="session"),
        ...
        html.Div(
            ...
            children=[
                html.Div([
                    ...
                    html.P("Loading NYC DOT Toolkit…", className="text-xs text-muted-foreground"),
        ...
        html.Div([
            dbc.Button([
                ...
                "AI Assistant"
            ], id="ai-toggle-btn", color="primary", className="rounded-pill shadow-lg nyc-animate-fade-up"),
        ], style={"position": "fixed", "bottom": "30px", "right": "30px", "zIndex": 1000}),

        dbc.Modal([
            dbc.ModalHeader(dbc.ModalTitle("🤖 NYC Data AI Agent")),
            dbc.ModalBody([
                html.Div(id="ai-chat-history", style={"height": "350px", "overflowY": "auto", "padding": "10px"}, className="mb-3"),
                dbc.InputGroup([
                    dbc.Input(id="ai-user-input", placeholder="Ask about NYC data..."),
                    dbc.Button("Send", id="ai-send-btn", color="primary"),
                ])
            ]),
        ], id="ai-modal", is_open=False, size="lg"),
    ],
)
...
def handle_ai_chat(n, message, history):
    ...
    user_msg = html.Div([
        html.B("You: "), html.Span(message)
    ], className="mb-2 p-2 bg-secondary-nyc rounded")
    ...
    bot_msg = html.Div([
        html.B("AI: "), html.Span(response)
    ], className="mb-2 p-2 bg-primary rounded border-nyc")
