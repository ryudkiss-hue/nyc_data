...
layout = dbc.Container([
    html.Div([
        ...
        html.P("Write and execute SQL directly against DuckDB. Results render in an interactive AG Grid with one-click CSV export.", className="nyc-page-sub"),
    ], className="nyc-page-header"),

    dbc.Row([
        dbc.Col([
            html.Label("Table", style={"fontSize": "0.78rem", "fontWeight": 600, "color": "var(--text-muted)"}),
            dcc.Dropdown(id="soql-table-sel", placeholder="Choose a table…",
                         style={"background": "var(--bg-secondary)"}),
        ], md=4),
        dbc.Col([
            html.Label("Example queries", style={"fontSize": "0.78rem", "fontWeight": 600, "color": "var(--text-muted)"}),
            dcc.Dropdown(id="soql-example-sel",
        ...
        dbc.Col([
            ...
            dbc.Button("▶ Run", id="soql-run-btn", color="success", size="sm", className="me-2"),
            dbc.Button("💾 Save to DuckDB", id="soql-save-btn", color="secondary", outline=True, size="sm"),
        ], md=3),
    ], className="mb-2"),
    ...
    dbc.Row([
        ...
        dbc.Col(dbc.Button("📊 Chart result", id="soql-chart-btn", color="primary", outline=True, size="sm"), md=4, style={"textAlign": "right"}),
    ], className="mb-2"),
...
def insert_template(example, table):
    if not example: return dash.no_update
...
def run_query(_, sql):
    if not sql or not sql.strip():
        return dash.no_update, dash.no_update, dbc.Alert("Enter a query.", color="warning"), dash.no_update
    try:
        df = db.query_df(sql.strip())
        if df.empty:
            return html.P("Query returned 0 rows.", style={"color": "var(--text-muted)"}), \
        ...
        meta = html.Span(f"{len(df):,} rows × {len(df.columns)} cols", className="nyc-pill nyc-pill-blue")
        return grid, meta, html.Div(), df.to_dict("records")
    except Exception as e:
        return dash.no_update, html.Div(), dbc.Alert(f"❌ {e}", color="danger", dismissable=True), {}
...
def auto_chart(_, records, theme):
    if not records: return html.Div()
    ...
    if not num: return dbc.Alert("No numeric columns to chart.", color="warning")
    if cat:
