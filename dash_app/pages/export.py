...
dash.register_page(__name__, path="/export", name="Export", order=10)
...
layout = dbc.Container([
    html.Div([
        ...
        html.P("Export any DuckDB table to CSV, Parquet, JSON, Excel, SQL DDL, or Markdown.", className="nyc-page-sub"),
    ], className="nyc-page-header"),

    dbc.Row([
        dbc.Col([
            html.Label("Dataset", style={"fontSize": "0.78rem", "fontWeight": 600, "color": "var(--text-muted)"}),
            dcc.Dropdown(id="exp-table-sel", placeholder="Select DuckDB table…", style={"background": "var(--bg-secondary)"}),
        ], md=4),
        dbc.Col([
            html.Label("Format", style={"fontSize": "0.78rem", "fontWeight": 600, "color": "var(--text-muted)"}),
            dcc.Dropdown(id="exp-format-sel", options=FORMATS, value="csv", clearable=False, style={"background": "var(--bg-secondary)"}),
        ], md=3),
        dbc.Col([
    ...
    html.H5("👁️ Preview (first 20 rows)", style={"fontWeight": 700, "marginBottom": "10px", "color": "var(--text-heading)"}),
...
@callback(Output("exp-preview", "children"), Input("exp-table-sel", "value"), prevent_initial_call=True)
def preview(table):
    if not table: return html.Div()
    df = db.query_df(f'SELECT * FROM "{table}" LIMIT 20')
    return dag.AgGrid(rowData=df.to_dict("records"),
...
def do_export(_, table, fmt, max_rows):
    ...
    try:
        ...
        if fmt == "csv":
            return dcc.send_data_frame(df.to_csv, f"{fname}.csv", index=False), ok(f"✅ {len(df):,} rows → CSV")
        if fmt == "parquet":
            buf = io.BytesIO(); df.to_parquet(buf, index=False); buf.seek(0)
            return dcc.send_bytes(buf.read, f"{fname}.parquet"), ok(f"✅ {len(df):,} rows → Parquet")
        if fmt == "json":
            return dcc.send_string(df.to_json(orient="records", indent=2), f"{fname}.json"), ok(f"✅ {len(df):,} rows → JSON")
        if fmt == "excel":
            buf = io.BytesIO(); df.to_excel(buf, index=False); buf.seek(0)
            return dcc.send_bytes(buf.read, f"{fname}.xlsx"), ok(f"✅ {len(df):,} rows → Excel")
        if fmt == "sql":
            type_map = {"int64":"BIGINT","float64":"DOUBLE","object":"VARCHAR","bool":"BOOLEAN"}
            ddl_cols = ",\n  ".join(f'"{c}" {type_map.get(str(df[c].dtype),"VARCHAR")}' for c in df.columns)
