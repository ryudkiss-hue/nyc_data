...
layout = dbc.Container([
    html.Div([
        ...
        html.P("Generate formatted Markdown reports from DuckDB data and download them.", className="nyc-page-sub"),
    ], className="nyc-page-header"),

    dbc.Row([
        dbc.Col([
            html.Label("Dataset", style={"fontSize": "0.78rem", "fontWeight": 600, "color": "var(--text-muted)"}),
            dcc.Dropdown(id="rep-table-sel", placeholder="Select DuckDB table…", style={"background": "var(--bg-secondary)"}),
        ], md=4),
        dbc.Col([
            html.Label("Report type", style={"fontSize": "0.78rem", "fontWeight": 600, "color": "var(--text-muted)"}),
            dcc.Dropdown(id="rep-type-sel", options=REPORT_TYPES, value="executive", clearable=False, style={"background": "var(--bg-secondary)"}),
        ], md=4),
        dbc.Col([
            html.Label("Author / Agency", style={"fontSize": "0.78rem", "fontWeight": 600, "color": "var(--text-muted)"}),
            dbc.Input(id="rep-author", value="NYC DOT", size="sm"),
        ], md=3),
        dbc.Col([html.Div(style={"height": "22px"}),
                 dbc.Button("📝 Generate", id="rep-gen-btn", color="primary", size="sm")], md=1),
    ], className="mb-3"),
    ...
    dbc.Row([
        dbc.Col(html.H5("📄 Report Preview", style={"fontWeight": 700, "color": "var(--text-heading)"}), md=8),
        dbc.Col(dbc.Button("⬇️ Download .md", id="rep-dl-btn", color="secondary", outline=True, size="sm"), md=4, style={"textAlign": "right"}),
    ], className="mb-2"),
...
def generate_report(_, table, rtype, author):
    ...
    try:
        ...
        ), style={"background": "var(--bg-secondary)", "border": "1px solid var(--border-color)", "maxHeight": "600px", "overflowY": "auto"})
        return preview, dbc.Alert("✅ Report generated", color="success", dismissable=True, duration=3000), md
...
def download_report(_, md, table):
    if not md: return dash.no_update
...
"""
    ...
    stats = "\n".join(
        f"| {c} | {df[c].mean():,.2f} | {df[c].min()} | {df[c].max()} |"
        for c in num_cols[:8]
    )
