...
if (Test-Path $requirementsPath) {
    ...
    if ($requirementsContent -notcontains "shapely>=2.0") {
        ...
        Write-Host "Added shapely>=2.0 to requirements-dev.txt"
    }
}
Write-Host ""
...
# ---------------------------------------------------------
# WRITE spatial.py MODULE
# ---------------------------------------------------------
Write-Host "Writing cleaned spatial.py module..."
if (!(Test-Path "socrata_toolkit")) {
    New-Item -ItemType Directory -Path "socrata_toolkit" | Out-Null
}
...
Set-Content -Path "socrata_toolkit/spatial.py" -Value $spatialCode
