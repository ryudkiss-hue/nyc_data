...
mantine_imports = '''import { useSQLQuery } from "@motherduck/react-sql-query";
...
def refactor_dive(file_path):
    ...
    desc_match = re.search(r'<p className="text-sm.*?>(.*?)</p>', content)
    desc = desc_match.group(1) if desc_match else "{meta.description}"
    
    # We will build a new JSX component
    new_component = f'''{mantine_imports}
