...
templates = {
    ...
    "D": ("Priority & Scaling Command Center", "Operational Adoption and Outlier Concentration"),
    "C": ("Inventory Segmentation Command Center", "Ramp Type Certainty and Distribution Balance"),
    "B": ("Resource Allocation Command Center", "Spatial Clustering and Completion Confidence")
}
...
def generate_dashboard(phase, title, subtitle):
    ...
    meta = {
        ...
        "description": f"Automated Phase {phase} executive dashboard: {subtitle}.",
        "requiredResources": [{ "name": "nyc_mission_control", "alias": "nyc_mission_control" }]
    }
  ...
  if (isLoading) {{
    return (
      <div style={{{{ padding: "100px", textAlign: "center", fontFamily: "sans-serif" }}}}>
        ...
        <h2 style={{{{ marginTop: "24px", color: "#666" }}}}>SYNCHRONIZING PHASE {phase} TELEMETRY...</h2>
      </div>
    );
  }}
  ...
  return (
    <div style={{{{ background: "#FFFFFF", color: "#000000", padding: "32px", fontFamily: "sans-serif", minHeight: "100vh" }}}}>
      <div style={{{{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "40px", borderBottom: "4px solid #005696", paddingBottom: "20px" }}}}>
        <div>
          <h1 style={{{{ margin: 0, fontSize: "32px", fontWeight: 900, textTransform: "uppercase", letterSpacing: "1px" }}}}>{title}</h1>
          <p style={{{{ color: "#666", fontSize: "18px", marginTop: "8px" }}}}>Phase {phase}: {subtitle}</p>
        </div>
      ...
      <div style={{{{ marginTop: "60px", borderTop: "1px solid #EEE", paddingTop: "20px", textAlign: "center" }}}}>
        <p style={{{{ fontSize: "12px", color: "#AAA", fontWeight: 600, letterSpacing: "1px" }}}}>
          NYC DOT SIM DASHBOARD · PHASE {phase} COMMAND CENTER · CONFIDENTIAL
        </p>
      </div>
    </div>
  );
...
for phase, (title, subtitle) in templates.items():
    generate_dashboard(phase, title, subtitle)

print("Generated 4 high-level automated Command Center dashboards.")
