...
def main():
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        dirs[:] = [d for d in dirs if d not in ['.git', '.venv', '__pycache__', '.pytest_cache', 'venv', 'node_modules', '.roo']]
            ...
            if module_path is None:
                ...
                for symbol in symbols:
                    if symbol not in exports:
                        ...
                        import_map[(source_module, symbol)].append(python_file.replace(base_dir, ''))
    ...
    report = {
        'summary': {
            ...
            'unique_missing_symbols': len(set((m, item['symbol']) for m, items in all_missing.items() for item in items))
        },
        'stub_modules': {
            module: {
