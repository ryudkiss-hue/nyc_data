...
APP = Path("app.py")
APP.write_text(
    open("socrata_toolkit/viz/dashboard.py", encoding="utf-8")
    .read()
    .replace('if __name__ == "__main__":\n    main()', "main()")
    .replace("from ..analysis.program import", "from socrata_toolkit.analysis.program import")
    .replace(
        "from ..engineering.borough_analysis import",
        "from socrata_toolkit.engineering.borough_analysis import",
    )
    .replace("from ..tools.tasks import", "from socrata_toolkit.tools.tasks import")
    .replace(
    ...
    .replace("from ..core.client import", "from socrata_toolkit.core.client import")
    .replace("from ..analysis.core import", "from socrata_toolkit.analysis.core import")
    .replace("from ..governance.core import", "from socrata_toolkit.governance.core import")
    .replace("from ..reports.reporting import", "from socrata_toolkit.reports.reporting import")
    .replace(
