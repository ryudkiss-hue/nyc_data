...
def _make_pack(tmp_path: Path) -> Path:
    ...
    (pack / "executive_summary.md").write_text(
        "# Executive Summary\n\nHello world.", encoding="utf-8"
    )
    (pack / "program_kpi.json").write_text(
        json.dumps({"overall_health": "green"}), encoding="utf-8"
    )
    return pack
...
def test_publish_dry_run_generates_report(tmp_path: Path):
        ...
        yaml.dump(
            {
                ...
                "bi_export": {
                    "enabled": True,
                    "dest_root": str(tmp_path / "bi"),
                    "include": ["program_kpi.json"],
                },
    ...
    report = publish_pack(
        pack_dir=pack, profile_path=profile, dry_run=True, state_path=str(tmp_path / "state.json")
    )
    d = report.to_dict()
