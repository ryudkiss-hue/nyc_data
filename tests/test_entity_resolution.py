...
class TestExactMatch:
    ...
    def test_exact_match_partial(self):
        ...
        matcher = ExactMatch(fields=["id", "name"], field_weights={"id": 0.6, "name": 0.4})
...
class TestFuzzyMatch:
    ...
    def test_fuzzy_match_word_order(self):
        ...
        matcher = FuzzyMatch(fields=["address"], algorithm="token_set_ratio")
...
class TestGeographicMatch:
    ...
    def test_geographic_match_within_threshold(self):
        ...
        matcher = GeographicMatch(
            lat_field="latitude", lon_field="longitude", distance_threshold_m=20.0
        )
...
class TestTemporalMatch:
    ...
    def test_temporal_match_overlapping_ranges(self):
        ...
        matcher = TemporalMatch(start_field="start_date", end_field="end_date")
        record1 = {"start_date": "2024-01-01", "end_date": "2024-03-31"}
        record2 = {"start_date": "2024-02-01", "end_date": "2024-04-30"}
...
class TestCompositeMatch:
    ...
    def test_composite_match_weighted(self):
        ...
        composite = CompositeMatch([(exact, 0.4), (fuzzy, 0.6)])
...
class TestDeduplicator:
    ...
    def test_manual_duplicate_marking(self):
        ...
        success = dedup.mark_as_duplicates(
            ["rec1", "rec2", "rec3"], canonical_id="rec1", reason="Manually identified"
        )
...
class TestMasterDataManager:
    ...
    def test_create_master_entity(self):
        ...
        entity_id = mgr.create_master_entity(
            record1, record2, entity_type="person", merge_strategy=EntityMergeStrategy.PICK_FIRST
        )
    ...
    def test_merge_pick_latest(self):
        ...
        entity_id = mgr.create_master_entity(
            record1, record2, entity_type="person", merge_strategy=EntityMergeStrategy.PICK_LATEST
        )
    ...
    def test_validate_merge(self):
        ...
        entity_id = mgr.create_master_entity(
            {"id": "1", "name": "John", "email": "john@example.com"}, entity_type="person"
        )

        is_valid, issues = mgr.validate_merge(entity_id, required_fields=["name", "email"])
...
class TestIncrementalMatcher:
    ...
    def test_match_new_record_auto_assign(self):
        ...
        mgr = MasterDataManager()
        matcher = IncrementalMatcher(mgr, auto_assign_threshold=0.95)

        # Create master
        mgr.create_master_entity({"id": "master_1", "name": "John Doe"}, entity_type="person")
...
class TestReviewWorkflow:
    ...
    def test_add_and_review_case(self):
        ...
        success = workflow.submit_decision(
            case_id, ReviewDecision.MATCH, reviewer="reviewer1", notes="Names are similar"
        )
    ...
    def test_reviewer_metrics(self):
        ...
        for i in range(5):
            case = ReviewCase(
                record1={"id": f"1_{i}"}, record2={"id": f"2_{i}"}, matching_score=0.8
            )
            case_id = workflow.add_case(case)
            workflow.assign_case(case_id, "reviewer1")
            workflow.submit_decision(case_id, ReviewDecision.MATCH, reviewer="reviewer1")
...
class TestReconciler:
    ...
    def test_reconcile_with_external(self):
        ...
        mgr.create_master_entity(
            {"id": "local_1", "name": "John Doe", "borough": "Manhattan"}, entity_type="person"
        )

        # Import external
        external_data = [{"id": "ext_1", "name": "John Doe", "borough": "Manhattan"}]
...
class TestRelationshipGraph:
    ...
    def test_add_relationship(self):
        ...
        rel_id = graph.add_relationship("block_1", "segment_1", RelationshipType.CONTAINS)
...
class TestPerformanceBenchmarks:
    ...
    def test_blocking_efficiency(self):
        ...
        start = time.time()
        blocker.create_candidate_pairs(records)
        elapsed = time.time() - start
...
class TestEndToEndFlow:
    ...
    def test_complete_deduplication_workflow(self):
        ...
        for group in groups:
            source_records = [
                records[int(rid)] for rid in group.duplicate_record_ids if rid.isdigit()
            ]

            if source_records:
                entity_id = mgr.create_master_entity(*source_records, entity_type="person")
...
class TestEdgeCases:
    ...
    def test_empty_records(self):
        ...
        dedup = Deduplicator()
        rule = DeduplicationRule(
            rule_id="test", entity_type="test", matching_strategy=ExactMatch(fields=["id"])
        )
