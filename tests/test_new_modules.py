...
def test_scorecards_to_dataframe():
    df = pd.DataFrame({
        "contractor": ["A"], "actual_sqft": [100], "actual_spend": [5000],
        "days_worked": [5], "status": ["complete"],
    })
    cards = generate_scorecards(df)
...
def test_bot_contract_status():
    df = pd.DataFrame({"contract_id": ["C-1", "C-1", "C-2"], "status": ["Complete", "Pending", "Complete"]})
...
def test_bot_borough_backlog():
    df = pd.DataFrame({"borough": ["MANHATTAN", "MANHATTAN", "BROOKLYN"], "status": ["Pending Repair", "Complete", "Pending Repair"]})
    bot = BotAdapter(default_data=df)
...
def test_ingestion_result_struct():
    result = IngestionResult(total=50, sidewalk_related=50, critical_count=5,
                              high_count=10, boroughs={"MANHATTAN": 20}, task_board_items_created=0)
