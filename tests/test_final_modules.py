...
def test_rules_engine_evaluate():
    engine = RulesEngine()
    engine.add_rule(Rule("high_backlog", field="pending_count", operator=">", threshold=100, message="Backlog too high"))
...
def test_rules_engine_evaluate_dataframe():
    df = pd.DataFrame({"borough": ["MANHATTAN"] * 5, "status": ["Pending Repair"] * 3 + ["Complete"] * 2})
    engine = RulesEngine()
