...
def test_markov_stochastic_matrix_validation():
    ...
    # Valid matrix
    good_matrix = np.array([[0.9, 0.1], [0.0, 1.0]])
...
def test_mr_optimization_greedy_logic():
    ...
    def simple_benefit(a):
        return a.condition_index
...
def test_bayesian_poisson_regression_recovery():
    ...
    assert (
        result.hdi_intervals["Predictor_Effect"][0]
        < true_beta
        < result.hdi_intervals["Predictor_Effect"][1]
    )
