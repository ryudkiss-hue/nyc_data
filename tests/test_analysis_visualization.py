...
def test_statistical_summary_numeric(db):
    ...
    stats = summary.compute(table="analytics.raw_counts_summary", columns=["row_count"])
...
def test_statistical_summary_categorical(db):
    ...
    stats = summary.compute(table="analytics.sidewalk_repair_matrix", columns=["row_val"])
