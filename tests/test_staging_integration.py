    ...
    return pd.DataFrame(
        {
            ...
            "borough": np.random.choice(
                ["MANHATTAN", "BROOKLYN", "BRONX", "QUEENS", "STATEN_ISLAND"], 1000
            ),
            ...
            "status": np.random.choice(
                ["COMPLETED", "PENDING", "REJECTED"], 1000, p=[0.7, 0.2, 0.1]
            ),
        }
    )
...
class TestStagedWorkflow:
    ...
    def test_all_phases_with_realistic_data(
        self, staging_inspection_data, staging_spatial_data, staging_timeseries_data
    ):
        ...
        start = time.time()
        fig_c, narrative_c = AnalyticsEngine.chart_distribution_classification(
            {"data": staging_inspection_data}
        )
        ...
        # Phase D: Anomaly
        start = time.time()
        fig_d, narrative_d = AnalyticsEngine.chart_anomaly_detection(
            {"spatial": staging_spatial_data}
        )
        ...
        start = time.time()
        fig_e, narrative_e = AnalyticsEngine.chart_seasonal_decomposition(
            {"data": staging_timeseries_data}
        )
