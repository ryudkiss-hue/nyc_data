...
| `built` (cost columns) | ugc8-s3f6 | Unusable for budget | 2026-07-26 | Full-corpus verified: `totalcosttoconstruct` / `totalcostofrepairstreett` etc. are only **3.5–3.9% populated** across all 105,990 rows, and **0% for 2024 and 2025** (last meaningful year: 2023, 64.6%). Do NOT use `built` for budget-dollar reporting — use `capital_projects_dashboard` instead. `built` remains valid for repair counts/locations. |
| `capital_projects_dashboard` | fb86-vt7u | ⚠ Requires dedup | 2026-07-26 | **Authoritative source for DOT capital budget vs spend** (5,685 DOT rows, 100% populated, current to 2026-05). Each project repeats once per reporting period (830 projects × 10 periods) — summing unfiltered inflates the portfolio **~9× ($331B vs the true $36.3B)**. Always take the latest `reporting_period` only; use `VisualizationEngine._latest_capital_snapshot()`. |
