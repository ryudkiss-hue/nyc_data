    ...
    [
        html.Div(
            [
                ...
                html.P(
                    "Ask questions in plain English — converted to SQL and executed against DuckDB.",
        ...
        dbc.Alert(
            [
                html.Strong(
                    "✅ LangChain connected — OpenAI NL→SQL active"
                    if _HAS_LC
                    else "⚠️ LangChain not installed — using rule-based SQL assistant. "
                ),
                html.Span(" Install langchain-openai for full NL→SQL." if not _HAS_LC else ""),
            ],
            color="success" if _HAS_LC else "warning",
            dismissable=True,
        ),
        # Context selector
        dbc.Row(
            [
                dbc.Col(
                    [
                        ...
                        dcc.Dropdown(
                            id="ai-table-sel",
                            placeholder="Select DuckDB table for context…",
                ...
                dbc.Col(
                    [
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                {
                                    "label": "What are the top 10 boroughs by count?",
                                    "value": "top boroughs by count",
                                },
                                {
                                    "label": "Show me nulls per column",
                                    "value": "null count per column",
                                },
                                {
                                    "label": "What is the average of all numeric cols?",
                                    "value": "average of all numeric columns",
                                },
                                {
                                    "label": "Show recent records",
                                    "value": "show 20 most recent records",
                                },
                                {
                                    "label": "How many unique values per column?",
                                    "value": "unique values per column",
                                },
                            ],
...
def handle_chat(send_clicks, n_submit, clear_clicks, question, table, messages, token):
    ...
    if tid == "ai-clear-btn":
        return (
            [],
            [_system_msg("Chat cleared. Ask me anything about your DuckDB data.")],
            html.Div(),
        )
                ...
                result_grid = html.Div(
                    [
                        ...
                        dag.AgGrid(
                            ...
                            columnDefs=[
                                {"field": c, "sortable": True, "filter": True} for c in df.columns
                            ],
...
def _nl_to_sql(question: str, table: str | None) -> tuple[str, str]:
    ...
    if any(x in q for x in ["top", "most", "count", "group"]):
        ...
        cat = next(
            (c for c in cat_df.columns if cat_df[c].dtype == object),
            cat_df.columns[0] if len(cat_df.columns) > 0 else "col",
        )
        sql = (
            f'SELECT "{cat}", COUNT(*) AS cnt FROM {t} GROUP BY "{cat}" ORDER BY cnt DESC LIMIT 10'
        )
