...
@st.cache_data(ttl=86_400, show_spinner="Loading dismissal data from Socrata…")
def _load_dismissals_from_socrata(limit: int = 15_000) -> pd.DataFrame:
    try:
        df = fetch_dataset("dismissals", limit=limit)
        for date_col in ("inspection_date", "request_date", "assigned_date"):
...
# ---------------------------------------------------------------------------
# Dismissals & Correspondences tab
# ---------------------------------------------------------------------------
