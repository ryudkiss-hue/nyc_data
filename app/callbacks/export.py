...
_BORO_COLS = {"borough", "boro", "boroname", "boroughname"}


def _summary_figure(df, title):
    """Compact kaleido-renderable summary of a dataset for image exports."""
    import plotly.graph_objects as go

    boro_col = next((c for c in df.columns if c.lower() in _BORO_COLS), None)
    if boro_col:
        vc = df[boro_col].astype(str).str.upper().value_counts().head(10)
        fig = go.Figure(go.Bar(x=list(vc.index), y=list(vc.values), marker_color="#2563EB"))
        fig.update_layout(title=f"{title} — records by borough")
    else:
        fig = go.Figure(go.Indicator(mode="number", value=len(df),
                                     title={"text": f"{title} — total records"}))
    fig.update_layout(template="plotly_white", height=560, width=900)
    return fig
...
def register_export_callbacks(app, dm_instance):
    ...
    def handle_asset_export(n_clicks, filters):
        ...
        if export_type == "csv":
        ...
        elif export_type in ("png", "jpg"):
            try:
                img = _summary_figure(df, title).to_image(
                    format="jpeg" if export_type == "jpg" else "png", width=900, height=560
                )
                return dcc.send_bytes(img, f"{chart_id}_export.{export_type}"), no_update
            ...
            content = (
                f"# Autogenerated R snippet for {chart_id}\n"
                f'df <- read.csv("{chart_id}_export.csv")\n'
                "summary(df)\n"
            )
            return dcc.send_string(content, f"{chart_id}_script.R"), no_update
        elif export_type == "pbi":
            content = (
                f"// Power Query (M) snippet for {chart_id} — paste into Power BI's Advanced Editor\n"
                "let\n"
                f'    Source = Csv.Document(File.Contents("{chart_id}_export.csv"), [Delimiter=",", Encoding=65001]),\n'
