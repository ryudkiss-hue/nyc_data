...
class ImpactAnalysis:
    ...
    def estimate_downstream_impact(self, node_id: str) -> Dict[str, float]:
        ...
        for affected_id in downstream:
            ...
            try:
                ...
                paths = list(
                    nx.all_simple_paths(self.dag.graph, source=node_id, target=affected_id)  # type: ignore
                )
            ...
            for downstream_id in downstream:
                ...
                if downstream_node and downstream_node.node_type.value == "sink":
                    try:
                        paths = list(
                            nx.all_simple_paths(
                                self.dag.graph, source=node_id, target=downstream_id  # type: ignore
                            )
                        )
    ...
    def _generate_remediation_steps(
        ...
        if not affected_nodes:
            return ["Change has no downstream impact - proceed with caution"]
