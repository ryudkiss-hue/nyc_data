...
from socrata_toolkit.analysis import generate_semantic_network_map
    ...
    def render(self):
        ...
        with ui.card().classes('w-full border border-gray-200 shadow-sm'):
            with ui.row().classes('w-full items-center justify-between mb-4'):
                ui.label('🕸️ Toolkit Semantic Network Map').classes('text-xl font-bold text-gray-800 dark:text-gray-200')
                ui.button('Refresh Map', icon='refresh', on_click=self.plot_map).classes('bg-blue-600 text-white outline')
            
            self.map_container = ui.column().classes('w-full items-center bg-gray-50 dark:bg-gray-900 rounded p-2')
            self.plot_map()
