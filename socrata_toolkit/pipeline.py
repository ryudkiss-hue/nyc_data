...
def detect_changes(old_df: pd.DataFrame, new_df: pd.DataFrame, key: str | list[str]) -> Any:
    ...
    merged = old_df.merge(
        new_df, on=key, how="outer", indicator="_merge", suffixes=("_old", "_new")
    )
    ...
    new_cols = [
        c.replace("_old", "_new") for c in old_cols if c.replace("_old", "_new") in both.columns
    ]
...
def sync_dataset(
    domain: str, fourfour: str, db_path: str, table_name: str, updated_col: str, token: str = ""
) -> int:
    """
    ...
    if tqdm and (total_to_fetch is None or total_to_fetch > 0):
        ...
        pbar = tqdm(
            total=total_to_fetch, desc=f"📡 Streaming {fourfour}", unit="rows", bar_format=bar_fmt
        )
...
def vlookup(
    df: pd.DataFrame, other_df: pd.DataFrame, left_on: str, right_on: str, col: str
) -> pd.DataFrame:
...
def create_presentation(df: pd.DataFrame, title: str):
    ...
    (Path("outputs/reports") / "presentation_info.txt").write_text(
...
def postgis_add_geom(manager: DuckDBManager, table_name: str, lat_col: str, lon_col: str):
    manager.conn.execute(
        f"UPDATE {table_name} SET geom = ST_Point({lon_col}, {lat_col}) WHERE geom IS NULL;"
    )


def compute_hotspots(
    df: pd.DataFrame, lat_col: str = COL_LAT, lon_col: str = COL_LON, borough: str | None = None
) -> pd.DataFrame:
...
def stream_pipeline(
    ...
    try:
        ...
        for batch in client.fetch_json(domain, fourfour, max_rows=max_rows):
            ...
            if duckdb_exporter:
                cfg = targets["duckdb"]
                duckdb_exporter.upsert_batches(
                    [batch], table=cfg.get("table"), conflict_column=cfg.get("conflict_column")
                )
...
class ExcelWorkbookBuilder:
    ...
    def add_data_sheet(
        self, name: str, df: pd.DataFrame, freeze_panes: str = XL_FREEZE
...
class WorkflowStep:
    ...
    def __init__(
        self,
        name: str,
        action: Callable[[pd.DataFrame], pd.DataFrame],
...
class Workflow:
    ...
    def export_to_airflow(self) -> str:
        ...
        for step in self.steps:
            lines.append(
                f"    {step.name}_task = PythonOperator(task_id='{step.name}', python_callable={step.action.__name__})"
            )
        return "\n".join(lines)
