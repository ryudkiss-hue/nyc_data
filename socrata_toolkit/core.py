...
class SocrataClient:
    ...
    def parallel_fetch(self, domain: str, fourfour: str, limit: int, max_workers: int = 8) -> pd.DataFrame:
    ...
    def search_datasets(self, query: str, limit: int = 10, offset: int = 0) -> pd.DataFrame:
        ...
        resp = with_retries(lambda: requests.get("https://api.us.socrata.com/api/catalog/v1", params=params, headers=self._headers()))
        results = [r.get("resource", {}) for r in resp.json().get("results", [])]
        return pd.DataFrame(results)

    def search_catalog(self, search_context: str | None = None, filters: str | None = None, order: str | None = None, limit: int = 10, offset: int = 0) -> pd.DataFrame:
        """Advanced catalog search using SoQL-like filters."""
        params: dict[str, Any] = {"limit": limit, "$offset": offset}
        if search_context: params["q"] = search_context
...
def generate_data_dictionary(df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    return [{"column": c, "dtype": str(df[c].dtype), "null_pct": f"{df[c].isna().mean()*100:.1f}%"} for c in df.columns]
...
class DuckDBRepository:
    ...
    def upsert_dataframe(self, df: pd.DataFrame, conflict_column: str) -> int:
        ...
        if not table_exists:
            ...
            self.manager.conn.execute(f"CREATE UNIQUE INDEX IF NOT EXISTS idx_{table_name}_{conflict_column} ON {table_name} ({conflict_column})")
            return len(df)
        else:
...
class DuckDBExporter:
    ...
    def upsert_batches(self, batches: Iterable[list[dict[str, Any]] | pd.DataFrame], table: str, conflict_column: str) -> int:
