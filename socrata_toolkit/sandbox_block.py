...
class SidewalkSandboxBlock:
            ...
            with ui.tab_panels(tabs, value=sandbox_tab).classes('w-full bg-transparent p-0'):
                with ui.tab_panel(sandbox_tab).classes('p-0'):
                    with ui.row().classes('w-full items-center justify-between mb-4'):
                        ui.label('📐 Sidewalk Anatomy Sandbox').classes('text-xl font-bold text-gray-800 dark:text-gray-200')
                        
                        with ui.dropdown_button('Export Options', icon='download', auto_close=True).classes('bg-blue-600 text-white outline'):
                            ...
                            ui.item('Export Data (.xlsx)', on_click=lambda: self.export_data('xlsx'))
                            ui.item('Export Data (.json)', on_click=lambda: self.export_data('json'))
                    ...
                    with ui.row().classes('w-full gap-4'):
                        ...
                        with ui.column().classes('w-1/3 bg-gray-50 dark:bg-gray-800 p-4 rounded shadow-inner'):
                            ui.label('Zone Widths (ft)').classes('font-bold text-lg mb-2')
                            
                            self._create_zone_controls('Frontage Zone', self.anatomy.frontage_zone)
                            self._create_zone_controls('Pedestrian Clear Path', self.anatomy.pedestrian_zone)
                            self._create_zone_controls('Furniture/Planting', self.anatomy.furniture_zone)
                            self._create_zone_controls('Curb Zone', self.anatomy.curb_zone)
        
                            ui.separator().classes('my-4')
                            ui.label('Live ADA Evaluation').classes('font-bold text-lg mb-2')
                            self.ada_status_container = ui.column().classes('w-full')
                            self._update_ada_readout()
                            
                            ui.separator().classes('my-4')
                            ui.label('Cost Estimate').classes('font-bold text-lg mb-2')
                        ...
                        with ui.column().classes('w-3/5 flex-grow'):
                            ...
                            self.chart = ui.plotly(self._generate_current_figure()).classes('w-full h-[500px]')
                            
                with ui.tab_panel(config_tab).classes('p-4 bg-gray-50 dark:bg-gray-800 rounded shadow-inner'):
                    ui.label('Global Material Unit Costs ($/sqft)').classes('text-xl font-bold mb-4')
                    ui.markdown('Adjust the baseline unit costs for each material. Changes are cached globally and reflect instantly in the Sandbox tab.')
                    ...
                    with ui.grid(columns=2).classes('w-full gap-4 max-w-3xl mt-4'):
                        for mat in SIDEWALK_MATERIALS.keys():
                            ...
                            with ui.row().classes('w-full items-center justify-between p-2 border border-gray-200 rounded bg-white dark:bg-gray-900'):
                                ui.label(mat).classes('font-semibold text-sm w-1/2')
                                ui.number(value=val, format='%.2f', step=0.5, 
                                          on_change=lambda e, m=mat: self._update_single_cost(m, e.value)).classes('w-1/3')
    ...
    def _create_zone_controls(self, label_text: str, zone_obj: SidewalkZone):
        ...
        with ui.column().classes('w-full mb-4'):
            ...
            with ui.row().classes('w-full items-center gap-2'):
                slider = ui.slider(min=0.0, max=15.0, step=0.5, value=zone_obj.width_ft,
                                   on_change=lambda e, z=zone_obj: self._on_width_change(z, e.value)).classes('flex-grow')
                ui.label().bind_text_from(slider, 'value', backward=lambda v: f"{v:.1f}'").classes('text-sm font-mono w-8')
            
            material_opts = list(SIDEWALK_MATERIALS.keys())
            ui.select(material_opts, value=zone_obj.material, label='Material',
                      on_change=lambda e, z=zone_obj: self._on_material_change(z, e.value)).classes('w-full text-xs')
    ...
    def _get_cost_data(self):
        ...
        zones = [self.anatomy.frontage_zone, self.anatomy.pedestrian_zone, self.anatomy.furniture_zone, self.anatomy.curb_zone]
        data = []
        total_cost = 0.0
        current_costs = self.get_costs()
        ...
        for z in zones:
            if z.width_ft > 0:
                ...
                total_cost += cost
                data.append({
                    "Zone": z.name, "Material": z.material, 
                    "Area (sqft)": f"{area:.1f}", "Est. Cost": f"${cost:,.2f}"
                })
    ...
    def _update_cost_readout(self):
        ...
        with self.cost_table_container:
            if data:
                ui.aggrid({'columnDefs': [{'field': k} for k in data[0].keys()], 'rowData': data}).classes('w-full h-40')
                ui.label(f'Total Estimate: ${total_cost:,.2f}').classes('text-lg font-bold mt-2 text-right w-full text-blue-700 dark:text-blue-400')
            else:
    ...
    def export_pdf(self):
        ...
        try:
            ...
            if saved_paths:
                ...
                ui.notify('PDF export failed. Check if "kaleido" package is installed.', type='warning')
    ...
    def export_ifa_memo(self):
        ...
        if not DOCX_AVAILABLE:
            ui.notify("python-docx is not installed. Run 'pip install python-docx'", type="negative")
            return
        ...
        doc.add_paragraph(f"Total Width: {self.anatomy.total_width_ft} ft | Length: {self.anatomy.length_ft} ft")
