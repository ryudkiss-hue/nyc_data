...
Generates a ``socrata_toolkit.core.config.json`` file and an ``.env.socrata``
environment file that the CLI and Python API auto-detect.

Run with::

    python -m socrata_toolkit.tools.wizard
"""
...
def main() -> None:
    ...
    # Save Configuration
    # -------------------------------------------------------------------
    cfg_path = root / "socrata_toolkit.core.config.json"
