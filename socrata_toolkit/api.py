...
Run standalone::
    ...
    flask --app socrata_toolkit.api run --port 5000

Or with gunicorn::

    gunicorn 'socrata_toolkit.api:create_app()' --bind 0.0.0.0:5000
"""
...
def create_app():
    ...
    def prioritize():
        import pandas as pd
        from .construction_list import prioritize_construction_list, classify_scope, flag_ada_locations, summarize_construction_list
        data = request.get_json()
        if not data or "rows" not in data:
            return jsonify({"error": "POST JSON with 'rows' array"}), 400
        df = pd.DataFrame(data["rows"])
