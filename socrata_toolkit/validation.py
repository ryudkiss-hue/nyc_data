...
MATERIAL_CATEGORIES = {
    ...
    "permeable_surfaces": {"Pavers", "Pervious Concrete"},
...
def validate_defect_applicability(
    ...
    for idx, row in df.iterrows():
        ...
        if pd.isna(material) or pd.isna(defect):
            continue
        ...
        if applicable != "all" and material not in applicable:
            errors.append(
    ...
    return ValidationReport(
        valid=not errors, errors=errors, warnings=[], affected_records=invalid_count
    )
