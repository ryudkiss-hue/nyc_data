...
def standardize_boroughs(df: pd.DataFrame, col: str) -> pd.DataFrame:
    ...
    mapping = {
        "1": "MANHATTAN", "MN": "MANHATTAN", "NEW YORK": "MANHATTAN",
        "2": "BRONX", "BX": "BRONX",
        "3": "BROOKLYN", "BK": "BROOKLYN", "KINGS": "BROOKLYN",
        "4": "QUEENS", "QN": "QUEENS",
        "5": "STATEN ISLAND", "SI": "STATEN ISLAND", "RICHMOND": "STATEN ISLAND"
    }
    out = df.copy()
...
def standardize_bbl(df: pd.DataFrame, boro_col: str, block_col: str, lot_col: str, target_col: str = "bbl") -> pd.DataFrame:
...
def clean_column_names(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    out.columns = [re.sub(r'[^a-z0-9_]', '', c.lower().replace(' ', '_').replace('-', '_')) for c in out.columns]
    return out
