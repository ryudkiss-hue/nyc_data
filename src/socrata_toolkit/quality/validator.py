...
class QualityValidator:
    ...
    def validate(
        self, df: pd.DataFrame, suite_or_name=None, table_name: str = "dataset"
    ) -> ValidationResult:
        """Perform mandate-compliant data validation. suite_or_name may be an ExpectationSuite or a string table name."""
        if isinstance(suite_or_name, str):
            table_name = suite_or_name
        ...
        ev = Evidence(
            "quality_score", completeness, 75, f"Overall quality score is {completeness}/100."
        )
        if completeness >= 75:
            passed.append(ev)
        else:
            ...
            if abs(moments["skewness"]) > 3.0:
                failed.append(
                    Evidence(
                        ...
                        moments["skewness"],
                        3.0,
                        f"Extreme skewness detected in '{col}'.",
                    )
                )
            ...
            if moments["kurtosis"] > 10.0:
                failed.append(
                    Evidence(
                        ...
                        moments["kurtosis"],
                        10.0,
                        f"Extreme kurtosis (fat-tail risk) in '{col}'.",
                    )
                )
            ...
            if not is_normal and len(df[col].dropna()) > 30:
                failed.append(
                    Evidence(
                        ...
                        False,
                        True,
                        f"Column '{col}' failed Shapiro-Wilk normality test; parametric OLS may be invalid.",
                    )
                )
        ...
        if any(f.key.endswith("_kurtosis") for f in failed):
            status = ValidationStatus.FAIL  # Critical risks
        elif failed:
