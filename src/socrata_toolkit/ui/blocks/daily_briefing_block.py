...
from ...analysis import InsightsEngine, compute_sla_metrics, generate_executive_briefing_automated
from ...viz import gauge_chart
...
class DailyBriefingBlock:
        ...
        with ui.card().classes("w-full border-t-4 border-blue-600 shadow-lg"):
            with ui.row().classes("w-full items-center justify-between mb-4"):
                with ui.column().classes("gap-0"):
                    ui.label("📰 Morning Executive Briefing").classes(
                        "text-2xl font-bold text-gray-800 dark:text-gray-200"
                    )
                    ui.label(f"Generated: {datetime.now().strftime('%A, %B %d, %Y - %H:%M')}").classes(
                        "text-sm text-gray-500"
                    )

                with ui.row().classes("gap-2"):
                    ui.button(
                        "Export to Desktop", icon="download", on_click=self.export_to_desktop
                    ).classes("bg-green-600 text-white shadow-md")
                    ui.button(
                        "Compile Report", icon="auto_awesome", on_click=self.generate_briefing
            ...
            with self.report_container:
                ui.label('Click "Compile Report" to analyze the current workspace state.').classes(
    ...
    async def export_to_desktop(self):
        ...
        target_ds_name = (
            "defects" if "defects" in self.state.datasets else list(self.state.datasets.keys())[0]
        )
    ...
    async def generate_briefing(self):
        ...
        if not self.state.datasets:
            with self.report_container:
                ui.notify("No datasets loaded. Please fetch data first.", type="warning")
                ui.label(
                    "⚠️ Action Required: Load a dataset (e.g., 311 complaints or repairs) to generate a briefing."
        ...
        with self.report_container:
            with ui.row().classes("w-full gap-4"):
                with ui.card().classes(
                    "flex-grow bg-blue-50 dark:bg-blue-900/20 items-center p-4"
                ):
                    ui.label("Total Records").classes(
                        "text-sm font-semibold uppercase text-gray-500"
                    )
                    ui.label(f"{len(df):,}").classes("text-4xl font-bold text-blue-600")

                with ui.card().classes(
                    "flex-grow bg-green-50 dark:bg-green-900/20 items-center p-4"
                ):
                    ui.label("SLA Compliance Rate").classes(
                        "text-sm font-semibold uppercase text-gray-500"
                    )
                    ui.label(f"{sla_metrics.sla_compliance_rate}%").classes(
                        "text-4xl font-bold text-green-600"
                    )

                with ui.card().classes(
                    "flex-grow bg-red-50 dark:bg-red-900/20 items-center p-4"
                ):
                    ui.label("SLA Violations").classes(
                        "text-sm font-semibold uppercase text-gray-500"
                    )
                    ui.label(f"{sla_metrics.violation_count:,}").classes(
            ...
            with ui.row().classes("w-full gap-6 mt-4"):
                with ui.column().classes(
                    ...
                    for insight in text_insights:
                        ui.label(f"• {insight}").classes(
                            "mb-2 text-md text-gray-700 dark:text-gray-300"
                        )

                with ui.column().classes("flex-grow items-center"):
                    fig = gauge_chart(
                        sla_metrics.sla_compliance_rate,
                        target=95.0,
                        title="Operations Health vs Target",
                    )
