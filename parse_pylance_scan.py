...
try:
    with open('final_pylance_scan.json', 'r', encoding='utf-8', errors='ignore') as f:
        content = f.read()
        # Find JSON start
    ...
    improvement_pct = ((baseline - total_errors) / baseline * 100) if total_errors < baseline else ((total_errors - baseline) / baseline * 100)
